...
# 用内置参数（不读计算书）
python draw_section.py CD段 --template
```

参数从**天汉基坑设计软件**导出的计算书里解析：土层（编号/名称/层底埋深/C/φ）、
桩排（桩顶标高/桩长/间距/直径）、冠梁、放坡、地面超载、支护结构类型。
解析出的参数直接喂给生成器，图纸画在**演示图复制出来的模板**里 ——
图框块、23 个图层、文字样式都是标准图里那套，图签栏也按标准位置自动填。

> 支护类型目前只实现了**桩锚/悬臂桩**（`pile`）。放坡土钉、双排桩等
> 会识别出来但明确提示「还没有绘图程序」，不会画错。
